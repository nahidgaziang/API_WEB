...
import { Course, CourseMaterial, BankAccount, Transaction, Enrollment, User, Topic } from '../models';
...
export const uploadCourse = async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    try {
        ...
        const expectedPayment = Math.round(parseFloat(price) * 0.20);
        if (Math.round(parseFloat(upload_payment)) !== expectedPayment) {
            await transaction.rollback();
            res.status(400).json({ success: false, message: `Upload payment must be exactly 20% of the course price (${expectedPayment} Tk)` });
            return;
        }
        const topic = await Topic.findOne({ where: { name: title, is_active: true } });
        if (!topic) {
            await transaction.rollback();
            res.status(400).json({
                success: false,
                message: 'Invalid course topic. Topic either does not exist or is inactive.',
            });
            return;
        }
...
export const claimPayment = async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    try {
        ...
        if (txn.transaction_type === 'instructor_payment') {
            const enrollment = await Enrollment.findOne({
                where: { course_id: txn.reference_id, status: 'pending' },
                transaction,
            });
            if (enrollment) {
